export { default as ArrowLeft } from "./ArrowLeft.svelte";
export { default as ArrowRight } from "./ArrowRight.svelte";
export { default as Burger } from "./Burger.svelte";
export { default as Close } from "./Close.svelte";
export { default as Trash } from "./Trash.svelte";
export { default as CloseModal } from "./CloseModal.svelte";
export { default as CloseTab } from "./CloseTab.svelte";
export { default as Component } from "./Component.svelte";
export { default as Community } from "./Community.svelte";
export { default as Corner } from "./Corner.svelte";
export { default as Delete } from "./Delete.svelte";
export { default as Figma } from "./Figma.svelte";
export { default as FigmaColored } from "./FigmaColored.svelte";
export { default as Folder } from "./Folder.svelte";
export { default as Github } from "./Github.svelte";
export { default as Hand } from "./Hand.svelte";
export { default as Home } from "./Home.svelte";
export { default as Main } from "./Main.svelte";
export { default as Menu } from "./Menu.svelte";
export { default as Maximize } from "./Maximize.svelte";
export { default as MenuCorner } from "./MenuCorner.svelte";
export { default as Minimize } from "./Minimize.svelte";
export { default as Muted } from "./Muted.svelte";
export { default as Pencil } from "./Pencil.svelte";
export { default as Pencil2 } from "./Pencil2.svelte";
export { default as Plus } from "./Plus.svelte";
export { default as Restore } from "./Restore.svelte";
export { default as Reset } from "./Reset.svelte";
export { default as Save } from "./Save.svelte";
export { default as Save2 } from "./Save2.svelte";
export { default as Settings } from "./Settings.svelte";
export { default as Sync } from "./Sync.svelte";
export { default as UnMuted } from "./UnMuted.svelte";
export { default as Download } from "./Download.svelte";
export { default as Loader } from "./Loader.svelte";
export { default as RadioNormal } from "./RadioNormal.svelte";
export { default as RadioChecked } from "./RadioChecked.svelte";
export { default as GnomeTabClose } from "./GnomeTabClose.svelte";
export { default as GnomeMinimize } from "./GnomeMinimize.svelte";
export { default as GnomeMaximize } from "./GnomeMaximize.svelte";
export { default as GnomeRestore } from "./GnomeRestore.svelte";
export { default as GnomeClose } from "./GnomeClose.svelte";
export { default as GnomeMenu } from "./GnomeMenu.svelte";
export { default as GnomePlus } from "./GnomePlus.svelte";
export { default as GnomeFigma } from "./GnomeFigma.svelte";
