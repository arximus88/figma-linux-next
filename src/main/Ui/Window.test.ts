...
mock.module("electron", () => {
  return {
    app: { getPath: () => "/tmp", emit: mock(), getVersion: () => "0.0.0", getName: () => "test" },
    net: { request: mock() },
    ...
    BrowserWindow: class {
      id = 1;
      webContents = { id: 2, send: mock(), on: mock(), once: mock(), getURL: () => "http://figma.com" };
...
describe("Window Tab Routing", () => {
  ...
  describe("Bug 2: New File tab remained open after opening an existing file", () => {
    ...
    test("createFile still closes the New File tab (existing behavior, explicit assertion)", () => {
      const closeTabSpy = spyOn(windowInstance, "closeTab");
      const addTabSpy = spyOn(windowInstance, "addTab").mockReturnValue({ id: 999, loadUrl: mock() } as any);
