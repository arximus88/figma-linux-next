...
const tempUserData = path.join(os.tmpdir(), "figma-linux-test-" + Math.random().toString(36).substring(7));
...
test("Storage Initialization Benchmark", async () => {
    ...
    console.log(`Warm start (file exists) average: ${((endWarm - startWarm) / iterations).toFixed(4)}ms`);
