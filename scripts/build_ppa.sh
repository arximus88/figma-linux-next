...
workdir="build/figma-linux-next-${version}";
mkdir -p $workdir;
...
echo "figma-linux-next (${version}-${rev}ubuntu0) devel; urgency=medium" >> /tmp/tmp_changelog
echo "" >> /tmp/tmp_changelog
echo "  * Publish ${version} version" >> /tmp/tmp_changelog
echo "${notes}" >> /tmp/tmp_changelog
echo " -- Figma Linux Community <figma-linux-next@community.dev>  $(date -R)" >> /tmp/tmp_changelog
echo "" >> /tmp/tmp_changelog
...
cp -rf ./resources/figma-linux-next.desktop $workdir/resources;

docker build -t 4tqrgqe5yrgfd/figma-linux-next-ppa --build-arg FIGMA_LINUX_VERSION=${version} --build-arg FIGMA_LINUX_REV=${rev} -f ./docker/Build_ppa .
